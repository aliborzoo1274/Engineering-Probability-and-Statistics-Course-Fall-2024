...
def multiples_of_ten(start, stop):
  ...
  while (start % 10 != 0):
    start += 1
  x = np.arange(start, stop, 10, dtype=float)
  return x
...
def make_one_hot(x):
  ...
  """
  y = np.zeros((len(x), max(x) + 1), dtype=float)
  idx0 = np.arange(y.shape[0])
  y[idx0, x] = 1
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  """
  z = np.zeros((x.shape[0], x.shape[1], y.shape[2])).astype(x.dtype)
